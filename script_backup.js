...
function updateStatus() {
  ...
  for (j in controllers) {
	...
	// for arrows
	for (var pad = 1; pad <= 2; pad++) {
		var id = "svg[id='arrow" + id.toString() + "]";
		
	}
	var i = controller.axes[1];
	...
	points = points.join(" ");
	if (Math.abs(i) < 0.01) {
		$("svg[id='arrow1']").children("#U").hide();
	}
	else {
		$("svg[id='arrow1']").children("#U").show();
	}
	$("svg[id='arrow1']").children("#U").attr("points",points);
